...
const JobPage = () => {
  ...
  const hasApplied = applications?.some(app => 
    app.job_id === parseInt(id) && app.candidate_id === user?.id
  );
  
  // Check if deadline has passed
  const deadlinePassed = job?.deadline ? isDeadlinePassed(job.deadline) : false;
  
  // Can apply if: not recruiter, not their job, hasn't applied, deadline not passed, job is open
  const canApply = !isRecruiter && !isMyJob && !hasApplied && !deadlinePassed && job?.is_open;
