...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      // Fetch applications
      const applicationsResponse = await fetch(`http://localhost:5000/api/applications/user/${user.id}`);
      const applications = await applicationsResponse.json();

      // Fetch saved jobs
      const savedJobsResponse = await fetch(`http://localhost:5000/api/saved-jobs/${user.id}`);
      ...
      if (savedJobsResponse.ok) {
        setSavedJobs(savedJobsData.slice(0, 5)); // Get last 5 saved jobs
        setStats(prev => ({
          ...prev,
